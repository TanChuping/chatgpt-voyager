...
describe('Version Management', () => {
  ...
  describe('isVersionCompatible', () => {
    it('should accept versions meeting minimum requirement', () => {
      expect(isVersionCompatible('0.7.0', 'gpt-voyager.folders.v1')).toBe(true);
      expect(isVersionCompatible('0.7.5', 'gpt-voyager.folders.v1')).toBe(true);
      expect(isVersionCompatible('1.0.0', 'gpt-voyager.folders.v1')).toBe(true);
    });

    it('should reject versions below minimum requirement', () => {
      expect(isVersionCompatible('0.6.9', 'gpt-voyager.folders.v1')).toBe(false);
      expect(isVersionCompatible('0.5.0', 'gpt-voyager.folders.v1')).toBe(false);
    });
    ...
    it('should handle invalid version strings gracefully', () => {
      expect(isVersionCompatible('invalid', 'gpt-voyager.folders.v1')).toBe(false);
    });
  });

  describe('isSupportedFormat', () => {
    it('should recognize supported formats', () => {
      expect(isSupportedFormat('gpt-voyager.folders.v1')).toBe(true);
    });

    it('should reject unsupported formats', () => {
      expect(isSupportedFormat('unknown.format')).toBe(false);
      expect(isSupportedFormat('gpt-voyager.folders.v2')).toBe(false);
  ...
  describe('getCompatibilityInfo', () => {
    ...
    it('should handle invalid version strings', () => {
      const info = getCompatibilityInfo('invalid', 'gpt-voyager.folders.v1');
      expect(info.compatible).toBe(false);
  ...
  describe('FORMAT_VERSIONS', () => {
    ...
    it('should include v1 format', () => {
      expect(FORMAT_VERSIONS['gpt-voyager.folders.v1']).toBeDefined();
    });
  });
  ...
  describe('Real-world Scenarios', () => {
    it('should handle current extension version compatibility', () => {
      const info = getCompatibilityInfo(EXTENSION_VERSION, 'gpt-voyager.folders.v1');
      expect(info.compatible).toBe(true);
    });
    ...
    it('should prevent imports from future versions gracefully', () => {
      ...
      const info = getCompatibilityInfo(futureVersion, 'gpt-voyager.folders.v1');
    ...
    it('should handle version ranges correctly', () => {
      ...
      const format = 'gpt-voyager.folders.v1';
