    ...
    if (
      ...
      try {
        ...
        migrationsApplied.push(
          `${migration.fromVersion} 鈫?${migration.toVersion}: ${migration.description}`,
        );
      } catch (error) {
        throw new Error(
          `Migration failed (${migration.fromVersion} 鈫?${migration.toVersion}): ${error}`,
        );
      }
    }
  }
