...
describe('FormulaCopyService', () => {
  ...
  it('should copy ChatGPT KaTeX display formulas from annotation source', async () => {
    const clipboard = navigator.clipboard as unknown as { write?: unknown };
    clipboard.write = undefined;
    ...
    display.classList.add('katex-display');
    ...
    const annotation = document.createElementNS('http://www.w3.org/1998/Math/MathML', 'annotation');
    annotation.setAttribute('encoding', 'application/x-tex');
    annotation.textContent = '\\chi^2=\\sum \\frac{(O-E)^2}{E}';
    ...
    katexSpan.appendChild(mathml);
    katexSpan.appendChild(html);
    display.appendChild(katexSpan);
    ...
    expect(writeTextMock).toHaveBeenCalledWith('$$\\chi^2=\\sum \\frac{(O-E)^2}{E}$$');
