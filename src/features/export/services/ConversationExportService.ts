...
export class ConversationExportService {
  private static readonly REPORT_JSON_FORMAT = 'gpt-voyager.report.v1' as const;

  private static readonly CHAT_JSON_FORMAT = 'gpt-voyager.chat.v1' as const;
  ...
  private static async exportDocumentPDF(
    ...
    return {
      ...
      error: 'Document PDF export is not included in GPT-Voyager.',
    };
  }
  ...
  private static composeDocumentMarkdown(content: string, metadata: ConversationMetadata): string {
    ...
    const title = metadata.title?.trim() || 'Document';
  ...
  private static generateFilename(extension: string, title?: string): string {
    ...
    return `gpt-chat-${y}${m}${day}-${hh}${mm}${ss}.${extension}`;
  }
