...
export const loadMermaid = async (): Promise<typeof mermaidInstance> => {
  ...
  try {
    ...
    console.error('[GPT-Voyager] Failed to load Mermaid library:', error);
    return null;
  }
};
...
export const isMermaidCode = (code: string): boolean => {
  ...
  // Avoid parsing tiny/incomplete snippets while still allowing small useful diagrams.
  if (codeTrimmed.length < 12) return false;
  ...
  const keywords = [
    ...
    'quadrantChart',
    'requirementDiagram',
    'requirement', // v11: requirement(Diagram)? 鈥?shorter form
    'sankey-beta',
  ...
  if (lines.length < 2 && !codeTrimmed.includes(';')) return false;
...
const createStyles = () => {
  ...
  style.textContent = `
    ...
    .gv-mermaid-toggle {
      ...
      gap: 4px;
      background: var(--gpt-surface-container, rgba(0,0,0,0.05));
      border-radius: 8px;
      padding: 2px;
      border: 1px solid var(--gpt-outline-variant, rgba(0,0,0,0.1));
    }
    ...
    .gv-mermaid-toggle button {
      ...
      color: var(--gpt-on-surface-variant, #666);
    }
    
    .gv-mermaid-toggle button:hover {
      background: var(--gpt-surface-container-high, rgba(0,0,0,0.08));
    }
    
    .gv-mermaid-toggle button.active {
      background: var(--gpt-primary, #1a73e8);
      color: white;
    }
...
const renderMermaid = async (codeBlock: HTMLElement, code: string) => {
  ...
  try {
    ...
    try {
      ...
      svg = `
                <div style="padding: 24px; text-align: center; color: var(--gpt-on-surface-variant, #666);">
    ...
    if (!wrapper?.classList.contains('gv-mermaid-wrapper')) {
      ...
      // We look for .buttons container (newer ChatGPT) or .copy-button class
...
const getCodeBlockLanguage = (codeEl: Element): string | null => {
  const classLanguage = getLanguageFromCodeClass(codeEl);
...
const processCodeBlocks = () => {
  ...
  codeElements.forEach((codeEl) => {
    ...
    if (!getCodeBlockContainer(codeEl)) return;

    const codeText = codeEl.textContent || '';

    // Check the language label from ChatGPT's code block header
...
const initializeMermaid = async () => {
  ...
  console.log('[GPT-Voyager] Mermaid integration started');
};
