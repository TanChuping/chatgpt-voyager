...
const MIN_MARGIN = 8;
const FAB_ICON_PATH = 'icon-128.png';
const DRAG_MOVE_THRESHOLD = 4; // px 鈥?below this we treat pointerup as a click, not a drag
...
function getExtensionAssetUrl(path: string): string {
  const runtime =
    (globalThis as { browser?: { runtime?: { getURL?: (path: string) => string } } }).browser
      ?.runtime ??
    (globalThis as { chrome?: { runtime?: { getURL?: (path: string) => string } } }).chrome
      ?.runtime;
...
export function mountFloatingFab({
  ...
  btn.title = label;
  const iconUrl = getExtensionAssetUrl(FAB_ICON_PATH);
  ...
  btn.innerHTML = `
    ...
    <span class="${FLOATING_FAB_CLASS}__inner">
      <img class="${FLOATING_FAB_CLASS}__icon" src="${iconUrl}" alt="" draggable="false" aria-hidden="true" />
    </span>
  `;
  ...
  // Keyboard accessibility 鈥?Enter/Space should still open the panel when
