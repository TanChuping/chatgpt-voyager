 ...
 * and pasted into a new ChatGPT conversation. Both conversations are linked
...
const STYLE_ID = 'gpt-voyager-fork-style';
...
function getNewConversationUrlForCurrentAccount(): string {
  const gptPrefix = window.location.pathname.match(/^\/g\/[^/]+/)?.[0] || '';
  return `${window.location.origin}${gptPrefix || '/'}`;
}
...
function navigateToForkConversation(node: ForkNode): void {
  ...
  const fallbackUrl =
    ...
    `${window.location.origin}/c/${encodeURIComponent(node.conversationId)}`;
