...
export function fixBrokenBoldTags(root: HTMLElement) {
  ...
  while ((node = walker.nextNode())) {
    ...
    if (
      ...
      (parent.tagName === 'CODE' ||
        parent.tagName === 'PRE' ||
        parent.tagName === 'MATH-BLOCK' || // ChatGPT custom element
        parent.tagName === 'MATH-INLINE' || // ChatGPT custom element
  ...
  for (const startNode of textNodes) {
    ...
    for (let steps = 0; walker2 && steps < MAX_WALK; steps++) {
      if (walker2.nodeType === Node.TEXT_NODE) {
        ...
        // Text node without ** 鈥?part of the bold content
        ...
        // Unknown node type 鈥?stop to avoid unexpected behavior
        break;
      }
...
/**
 * Starts the observer to patch broken markdown rendering in ChatGPT
 */
