...
const STYLE_ID = 'gpt-voyager-quote-reply-style';
...
function isChatInputEmpty(input: HTMLElement | HTMLTextAreaElement): boolean {
  ...
  // ChatGPT currently uses Quill internals. `ql-blank` is its canonical empty marker.
...
export function startQuoteReply() {
    ...
    if (input) {
      ...
      const performInsertion = () => {
        ...
        if (input instanceof HTMLTextAreaElement) {
          ...
          // Contenteditable (ChatGPT/Quill) logic
      ...
      console.warn('[GPT-Voyager] Could not find chat input.');
    }
  }
  ...
  function handleSelectionChange() {
    ...
    selectionDebounceTimer = setTimeout(() => {
      ...
      // ChatGPT uses <main> or sometimes specific classes. We want to avoid nav, sidebar, etc.
